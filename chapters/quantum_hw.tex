...
both the \textbf{baseline validation}, which demonstrates that asymmetric SQUID qubits achieve state-of-the-art performance at conventional operating conditions, and the \textbf{transformative innovation}, which explores the frontier of elevated-temperature quantum computing by pushing operation to $300$~GHz at $4$--$10$~K.
...
\bottomrule
\end{tabular}
\caption{Technical-feasibility indicators at the innovation operating point.
All quantities meet or are within striking distance of the relevant threshold.}
\label{tab:tech_feasibility_4K}
\end{table}

\subsubsection{Baseline Validation: CONFIRMED}

The $5.8$~GHz, $20$~mK system achieves a $98.7\%$ CNOT fidelity, validating the asymmetric SQUID architecture and design methodology, providing confidence in the theoretical models, and de-risking the innovation phase.
...
Junction fabrication                    & advanced lithography at high current density \\
$300$~GHz control electronics           & leverage existing mm-wave astronomy technology \\
Thermal management                      & modern pulse-tube coolers \\
\bottomrule
\end{tabularx}
\caption{Technical challenges at the innovation operating point and proposed
mitigation strategies. All four are addressable with existing or near-existing
technology.}
...
\begin{tabularx}{\linewidth}{@{}l L@{}}
\toprule
\textbf{Metric} & \textbf{Improvement over dilution-refrigerator baseline} \\
\midrule
Cost                & $10\times$ reduction (\euro $75$\,k vs \euro $750$\,k--\euro $1.5$\,M) \\
Development time    & $24$--$100\times$ faster (hours vs days cool-down) \\
Power consumption   & $10\times$ lower ($0.5$~kW vs $5$~kW) \\
Scalability         & $10\,000+$ qubits per cooler (vs $\sim 1000$) \\
Market potential    & \euro $300$\,M--\euro $5$\,B in addressable value creation \\
\bottomrule
\end{tabularx}
\caption{Economic and operational impact of the HEATS-Q architecture relative to
the conventional dilution-refrigerator baseline.}
...
Asymmetric SQUID qubits at $5$--$6$~GHz achieve a $98.7\%$ CNOT fidelity at $20$~mK, with negligible thermal effects ($< 10^{-5}\%$). This is competitive with the state of the art and \textbf{demonstrates that the concept works}.

\subsubsection*{Part B: Innovation (Novel)}

Operation at $300$~GHz enables temperatures of $4$--$10$~K, with thermal populations remaining manageable ($3$--$10\%$) and projected fidelities above the error-correction thresholds, opening a path to qubit operation outside the dilution-refrigerator regime.
