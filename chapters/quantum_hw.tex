...
The two protocols offer complementary trade-offs, summarised in
Table~\ref{tab:gate_methods_comparison}. The ZZ-coupling gate is slower but
substantially simpler at the hardware level, while $\sqrt{i\text{SWAP}}$ is
faster but more demanding in control infrastructure.
...
\toprule
\textbf{Property} & \textbf{$\sqrt{i\text{SWAP}}$ (resonant)} & \textbf{ZZ-coupling (dispersive)} \\
\midrule
Gate time           & $\sim 80$~ns              & $\sim 224$~ns (Sec.~\ref{sec:gate_strategy} echo CR: $\sim 250$~ns) \\
Coupling strength   & strong, $g \sim 80$~MHz    & weaker, $J_{zz}/g \sim 0.1$ \\
Flux control lines  & required                  & not required (fixed-frequency qubits) \\
Hardware complexity & higher, more pulse stages & lower, fewer control lines \\
Leakage to $\ket{2}$ & possible near resonance    & negligible (always far-detuned) \\
Frequency-noise sensitivity & high               & low \\
Suitability above $1$~K     & poor                & good (no flux electronics required) \\
Scalability         & moderate                  & good \\
\bottomrule
\end{tabularx}
\caption{Comparison of the two two-qubit gate strategies considered for HEATS-Q.
The ZZ-coupling protocol is preferred for elevated-temperature operation;
$\sqrt{i\text{SWAP}}$ remains a faster alternative when flux-control infrastructure
is available.}
\label{tab:gate_methods_comparison}
\end{table}
...
although the gate time of $\sim 224$~ns is slightly above the conventional $200$~ns ``fast gate'' threshold, the predicted fidelity ($99.55\%$) clears the $99\%$ surface-code threshold by a comfortable margin, so the $0.29\%$ fidelity difference relative to $\sqrt{i\text{SWAP}}$ is negligible in practice.

\subsubsection*{Scalability}

For multi-qubit processors:
for multi-qubit processors the $\sqrt{i\text{SWAP}}$ approach requires fewer control lines ($N$ microwave lines instead of $N$ microwave plus $N$ flux lines), which reduces inter-qubit crosstalk and simplifies the cryogenic wiring at elevated temperatures.

\subsubsection*{Final considerations}

Both reported gate times are potentially valid choices:
the two reported gate times are both valid: $80$~ns corresponds to a $\sqrt{i\text{SWAP}}$ implementation using resonant coupling, while $224$~ns corresponds to a ZZ-coupling implementation using the dispersive interaction.
...
\toprule
\textbf{Property} & \textbf{Rationale} \\
\midrule
Flux-bias-free design  & compatible with the asymmetric SQUID architecture \\
Robustness at $4$--$6$~K & no flux-control electronics required at the cold stage \\
Hardware simplicity      & fewer control lines, simpler calibration \\
Performance              & achieves a CNOT fidelity around the target $99.5\%$ \\
Scalability              & better suited to large-scale architectures \\
\bottomrule
\end{tabular}
\caption{Reasons for adopting the cross-resonance strategy for HEATS-Q.}
\label{tab:heatsq_gate_rationale}
\end{table}
...
\begin{figure}[htbp!]
    ...
    (purple dashed line; Sec.~\ref{sec:cavity_mediated_couplings}). 
    ...
    $\chi_i/\kappa \gg 1$. \emph{Note}: the parameter box embedded in the figure
    PNG still labels the inter-qubit interaction as $\zeta_{12}/2\pi \simeq -7.0$~MHz, 
    a relic of the pre-G7 derivation; the value coincides numerically with the
    transverse exchange $J$ of Eq.~\ref{eq:J_dispersive} but the symbol/interpretation
    is updated here in the caption.}
...
\textbf{(d)} Dispersive parameters: dispersive shifts $|\chi_i|$ and the cavity-mediated transverse exchange $|J|$ (left axis, blue/cyan bars; the bar labelled \textit{J\_zz} in the figure PNG is the transverse exchange $J$ of Eq.~\ref{eq:J_dispersive}, retained with the legacy symbol from the pre-G7 plotting code), and dispersive ratios $|\Delta_i|/g$ (right axis, green bars). All values support quantum non-demolition readout and the echo-CR two-qubit gate of Sec.~\ref{sec:gate_strategy}.
