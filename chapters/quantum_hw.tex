...
The $\sqrt{i\text{SWAP}}$ approach uses \textit{resonant} coupling between the two qubits.

The protocol consists of five steps: \emph{(i)} flux pulses are applied to bring
the two qubit frequencies into resonance ($\omega_{q1} \approx \omega_{q2}$);
\emph{(ii)} the resulting strong exchange interaction (rate
$g_{\text{eff}} \approx g \sim 80$~MHz) is allowed to evolve for a time
$t = \pi/(4 g_{\text{eff}})$, performing a partial swap; \emph{(iii)} the qubits
are returned to their original frequencies; \emph{(iv)} single-qubit corrections
are then applied to obtain the desired entangling operation.
...
\textbf{Advantages:}
Its main advantages are a fast gate time ($\sim 80$~ns), a high coupling strength, the maturity of the technology (already used by Google and Rigetti), and a high experimentally demonstrated fidelity.

\textbf{Disadvantages.} The $\sqrt{i\text{SWAP}}$ gate requires flux-control lines for frequency tuning, uses more complex pulse sequences, exposes the system to potential leakage to $\ket{2}$ when near resonance, is sensitive to frequency fluctuations, and is therefore less robust at elevated temperatures.

\subsection{Gate-strategy redirection after the G7 review}
\label{sec:gate_redirection}
...
\label{sub:tqb_couplings}
% PLACEHOLDER — Wave I Step 2.C will migrate here:
%   §6 lines 1922--2070 (whole section: transverse J, longitudinal ζ_zz, erratum to Eq. 2.39).
% Action: KEEP. Existing labels sec:cavity_mediated_couplings (12 external refs)
% and subsec:erratum_eq239 (3 external refs) will be moved here with content.
...
% PLACEHOLDER — Wave I Step 2.C will migrate here:
%   §1 lines 1178--1221 (CNOT decomposition, eq:cnot_decomp).
% + §1 lines 1251--1276 (Gate-strategy redirection — merged as sub-section intro).
% + §7 lines 2072--2178 (CR Hamiltonian, optimization of detuning, echo-CR, fidelity budget).
% Action: KEEP+MERGE. DROP §1 lines 1222--1250 (Method 1 sqrt-iSWAP, abandoned method).
% Existing label sec:gate_strategy (16 external refs) and sub-labels
% subsec:CR_hamiltonian, subsec:dq_optimization, subsec:echoCR will be moved here.
% NOTE: the single internal \ref{sec:gate_redirection} (line ~1607) will be
% redirected to \ref{sub:tqb_twoq_cr} in Step 2.C — no alias label needed.
...
Sec.~\ref{sec:gate_redirection} and Fig.~\ref{fig:CZ_unfeasible}.
