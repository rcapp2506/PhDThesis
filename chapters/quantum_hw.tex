...
\textbf{1. Ultra-Low-Loss Substrates.} Sapphire ($\tan\delta < 10^{-7}$) or high-resistivity silicon, combined with a 3D cavity design that minimises the participation ratio, are expected to give $T_{1,\text{diel}} > 300~\mu$s.

\textbf{2. 3D Electromagnetic Cavity (Essential)}
Full shielding with a copper or aluminum cavity targets a $60$~dB suppression of radiative losses. The design uses a quarter- or half-wave coaxial cavity, and is expected to give $T_{1,\text{rad}} > 400~\mu$s.

\textbf{3. Reduced Participation Ratio}
The strategy is to maximize the capacitance to vacuum (rather than to a dielectric), exploiting the fact that a 3D architecture naturally achieves a participation ratio $p < 0.1$, possibly augmented by air-bridge capacitors.
...
The required junction diameter is $620$--$880$~nm.
The target parameters are a critical current $I_c = 30~\mu$A with $< 5\%$ uniformity, achieved at a current density $J_c = 1000$--$1500$~A/cm$^2$.
...
The required precision is achieved through e-beam lithography with $10$~nm resolution, the Dolan-bridge shadow-evaporation technique, and multiple test fabrication runs.

\textbf{2. High Current Density}
Control of the barrier thickness is obtained with thin barriers ($d_{\text{ox}} \sim 1$~nm), precise oxidation under in-situ monitoring, and ALD for uniformity.

\textbf{3. Junction Arrays}
An $N \times N$ array of smaller junctions (e.g.\ a $4 \times 4$ array of $250$~nm junctions) improves both yield and uniformity.

\textbf{4. Flux Tunability}
A SQUID loop, finally, allows $\pm 5\%$ in-situ frequency tuning to compensate for any residual fabrication variations.
...
The required components include $300$~GHz sources and amplifiers, low-noise cryogenic stages, and dedicated waveguide and transition components.
...
The relevant technology base is mature: ALMA-class instrumentation operates routinely at $300$~GHz, Virginia Diodes Inc.\ provides multiplier chains up to $1$~THz, and WR-3 ($220$--$325$~GHz) is a standard waveguide format.

\textbf{2. Frequency Conversion}
An alternative is to generate control pulses at $\sim 10$~GHz and use on-chip frequency multiplication ($\times 30$), combined with Josephson parametric amplifiers.

\textbf{3. Cost Estimate}
Indicative costs are $\sim$\euro$200$k for the $300$~GHz synthesizer, $\sim$\euro$150$k for amplifiers and multipliers, and another $\sim$\euro$150$k for the cryogenic components, for a \textbf{total control stack of $\sim$\euro$500$k}.
...
Temperature stability and noise requirements are stringent:
The thermal requirements are stability $< 1$~mK, careful thermal isolation of the qubit chip, and a low-noise environment.
...
A Cryomech PT420 (1~W cooling power at $4$~K) provides temperature stability $< 0.5$~mK and supports passive vibration isolation.

\textbf{2. Thermal Filtering}
Thermalization is multi-stage ($40$~K, $10$~K, $4$~K) with copper-braid links and Eccosorb microwave filters.
...
Achieving $J_c = 1000$--$1500$~A/cm$^2$ requires careful process control.
The fabrication targets are an oxide thickness $d_{\text{ox}} \approx 1.0$--$1.2$~nm, an oxidation time of $60$--$90$~s (pressure-dependent), and uniformity $< 5\%$ across the chip.
...
\midrule
E-beam lithography resolution & $10$~nm \\
Junction-formation technique  & Dolan-bridge (shadow evaporation) \\
Alignment accuracy            & $\pm 5$~nm \\
Critical-dimension control    & $\pm 20$~nm \\
\bottomrule
\end{tabular}
\caption{Lithographic requirements for the innovation transmon fabrication.}
\label{tab:litho_requirements}
\end{table}
...
\textbf{Challenge:} $300$~GHz operation requires demanding junction specifications.
The main fabrication challenges are the small junction area ($\sim 50$~nm diameter, $0.002~\mu$m$^2$), precise barrier thickness control ($< 1$~nm), and high uniformity across the chip.

\textbf{Status:} At the edge of current e-beam lithography capabilities

\textbf{Mitigation:}
Mitigation strategies include advanced fabrication techniques (e.g.\ EUV lithography), atomic layer deposition for barriers, and in-situ tuning mechanisms.

\subsubsection{Control and Readout}

\textbf{Challenge:} 300~GHz microwave engineering
The control and readout challenges are the limited choice of commercial components, the criticality of the waveguide design, and the difficulty of impedance matching.

\textbf{Status:} Established in millimeter-wave astronomy/radar

\textbf{Mitigation:}
Mitigation relies on leveraging existing $300$~GHz technology, on-chip frequency mixing, and hybrid approaches with control electronics at lower frequencies.
...
\textbf{Analysis:}
The decoherence channels scale differently with frequency: dielectric loss is $\propto \omega$ (worse), radiative loss is $\propto \omega^2$ (worse), and quasiparticle loss is $\omega$-independent.

\textbf{Mitigation:}
Mitigation strategies include ultra-low-loss materials (sapphire, silicon), 3D cavity designs to suppress radiative losses, improved shielding, and the side benefit that a higher operating temperature may even help $T_1$ by reducing quasiparticle generation.
\end{comment}
