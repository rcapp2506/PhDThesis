...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Substrate} & \textbf{Loss tangent $\tan\delta$} & \textbf{Resulting $T_{1,\text{diel}}$} \\
\midrule
Standard Si           & $\sim 10^{-5}$       & $< 1~\mu$s — unsuitable even with 3D cavities \\
High-resistivity Si   & $\sim 5\times 10^{-8}$ & $\sim 106~\mu$s — viable but more expensive \\
Sapphire (c-axis)     & $\sim 10^{-7}$ standard, $< 10^{-8}$ high-purity & $\sim 53~\mu$s standard, $> 530~\mu$s high-purity \\
\bottomrule
\end{tabular}
\caption{Substrate options for the HEATS-Q transmon. Sapphire offers the best
trade-off between availability, cost, and dielectric quality. The
high-resistivity Si values assume $\rho > 10~\text{k}\Omega\cdot\text{cm}$.}
