...
the second quantum revolution~\cite{DowlingMilburn2003, Deutsch2020}: a
transition from passively exploiting quantum phenomena (the transistor, the
laser) to actively engineering coherence and entanglement for computing and
sensing purposes (Fig.~\ref{fig:revolutions}).
...
\begin{figure}[h!]
  ...
  \caption{From observation to engineering of quantum phenomena.
    Adapted from Dowling \& Milburn~\cite{DowlingMilburn2003} and
    Deutsch~\cite{Deutsch2020}.}
...
50\,mK~\cite{krantz2019, kjaergaard2020}---expensive, slow to cool, and
difficult to scale.  Hybrid quantum-classical algorithms show promise for
near-term hardware~\cite{Preskill2018, biamonte2017, cerezo2021}, but the
nature and extent of their advantage over purely classical approaches is
still debated.  Single-photon detectors for fundamental-physics searches,
most notably axion haloscopes~\cite{Sikivie1983, Lamoreaux2013}, must
operate in strong magnetic fields where conventional devices suffer from
prohibitive noise.
...
We develop a system-level design for an asymmetric-SQUID transmon
qubit~\cite{Koch2007Transmon}, framed within circuit quantum
electrodynamics~\cite{Blais2004, blais2021circuit}, based on NbN/AlN/NbN
Josephson junctions and targeting operation at $\sim$\,4\,K, well beyond
the elevated-temperature transmon regime demonstrated to
date~\cite{Anferov2024HighTemp}.  The analysis proceeds in two stages: a baseline at
...
convolutional neural network (Q-CNN)~\cite{Mitarai2018, Sebastianelli2021}
for binary satellite-image classification on a subset of
EuroSAT~\cite{Helber2019}. The architecture, whose hyperparameters descend
from the architectural search of Ref.~\cite{Filippi2025Thesis}, inserts a
...
We construct a master-equation model of a Rydberg-atom~\cite{Saffman2010,
Adams2020} avalanche single-photon detector, building on the facilitated
excitation avalanches demonstrated in cold Rydberg
gases~\cite{Valado2016, Simonelli2016} and on their recent theoretical
characterisation~\cite{Nill2024}, motivated by the readout requirements of
axion-haloscope experiments~\cite{Sikivie1983, Lamoreaux2013}.  The model
tracks the spatiotemporal dynamics of a dipole-dipole-driven excitation
cascade across a chain of atoms, and its predictions are examined as a
function of an external magnetic field spanning 0--5\,T.  Sensitivity is
analysed through the quantum Fisher information
formalism~\cite{Giovannetti2011, Degen2017}, and the proposed scheme is
