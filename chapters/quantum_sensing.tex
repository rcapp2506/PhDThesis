...
The linear fit of Fig.~\ref{fig:N_scaling}(a) is a large-$N$ parametrisation obtained at $N \geq 11$, and its domain of validity deserves to be stated explicitly. Extending the exact coherent simulation --- same Hamiltonian, parameters and central-seed initial state, at $B=0$ --- down to $N = 1$--$14$ [Fig.~\ref{fig:smallN_readout}(a)] shows the amplification turning sub-linear below $N \approx 8$ and bending continuously to $\mathcal{A}(1) = 1$: a single atom has no neighbour to facilitate, hence no avalanche and no gain. The positive intercept of the fit ($a = 2.31$) is thus a feature of the large-$N$ parametrisation only: extrapolated below its domain it over-predicts the gain, crossing the detection threshold $\mathcal{A}=3$ already at $N \approx 2$--$3$, whereas the exact curves cross it at $N \gtrsim 4$ (peak readout) and $N \gtrsim 7$ (convention readout). The production sizes $N = 11$--$15$ sit safely inside the linear regime. Panel~(b) compares the two readout times: the convention $T_a = N/\Omega_{\mathrm{gr}}$ of Eq.~\eqref{eq:amplification_time} grows three times faster than the measured position of the first maximum of $S(t)$, which follows $\Omega_{\mathrm{gr}} T_a^{\mathrm{peak}} \approx 0.34\,N$ --- the end of the ballistic phase, when the two avalanche fronts reach the chain boundaries, and the natural operating point of a detector based on this scheme.

Strictly speaking, the coherent traces of Fig.~\ref{fig:B_field_scan}(a) do not saturate: after the first maximum the signal oscillates, since in a closed finite chain the excitation reflects off the boundaries and periodically refocuses (finite-size revivals). Solving the Lindblad master equation with pure dephasing, $L_j = \sqrt{\Gamma_\phi}\,\sigma_z^{(j)}$ on every site (QuTiP \texttt{mesolve}, exact up to $N=7$, cross-validated against an independent split-operator integrator), shows [Fig.~\ref{fig:smallN_readout}(c,d)] that moderate dephasing damps these revivals into a genuine plateau at $\mathcal{A}_{\mathrm{plateau}} \approx 0.42\,N$ --- the ``saturation'' of the closed-system simulations is the first maximum of a coherent oscillation, and becomes a true plateau only through dephasing --- while the early amplification peak, reached well before dephasing can act, is reduced by less than $10\%$ at $\Gamma_\phi = 0.1\,\Omega_{\mathrm{gr}}$ (e.g.\ $4.72 \to 4.36$ at $N=7$). At $\Gamma_\phi \sim \Omega_{\mathrm{gr}}$ peak and plateau merge, consistent with the $\Gamma_\phi T_a \sim 1$ crossover towards the classical, directed-percolation-like regime of driven Rydberg gases~\cite{Helmrich2020} and with the robustness to moderate dephasing reported experimentally~\cite{Festa2022}. The early, data-driven readout is therefore preferable on both counts --- largest signal, least exposure to decoherence --- whereas waiting until $N/\Omega_{\mathrm{gr}}$ costs signal in the dephased dynamics as well. Radiative and blackbody-induced decay during the avalanche are not included here and remain for future work; scripts and raw data are archived alongside the avalanche simulation code.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.92\textwidth]{chapters/qs_figures/smallN_readout_dephasing.png}
    \caption{Small-$N$ behaviour, readout time, and dephasing. \textbf{(a)} Amplification factor versus array size $N = 1$--$14$ (exact coherent simulation, $B=0$) for the two readout conventions: first maximum of $S(t)$ (red circles) and fixed convention time $T_a = N/\Omega_{\mathrm{gr}}$ (blue squares); the grey solid line is the production fit of Fig.~\ref{fig:N_scaling}(a) over its fitting domain $N \geq 11$, the grey dashed line its extrapolation below that domain, and the black dashed line the detection threshold $\mathcal{A} = 3$. \textbf{(b)} Readout time versus $N$: fixed convention (grey squares) and measured first maximum (blue circles), following $\Omega_{\mathrm{gr}} T_a \approx 0.34\,N$. \textbf{(c)} Total signal $S(t)$ at $N = 7$ under pure dephasing, $\Gamma_\phi/\Omega_{\mathrm{gr}} = 0, 0.1, 0.5, 1$ (Lindblad, QuTiP \texttt{mesolve}); markers indicate the maximum of each trace. \textbf{(d)} Peak amplification in the coherent limit and at $\Gamma_\phi = 0.1\,\Omega_{\mathrm{gr}}$, and late-time plateau amplification, versus $N = 2$--$7$.}
