...
\begin{enumerate}
    \item \textbf{Robust amplification in magnetic fields, and linear scaling with $N$}: The avalanche mechanism maintains amplification factor $\mathcal{A} \approx 5.3$ at $N=11$ atoms across magnetic fields from 0 to 5~T, despite interaction suppression from state mixing. Direct simulation at $N = 11, 13, 15, 17$ establishes a linear scaling $\mathcal{A}(N) = 2.31 + 0.286\,N$ that holds uniformly across the entire field range, with the four $B$ values overlapping to within $1\%$ at every $N$. Automatic compensation of Zeeman shifts through laser auto-detuning preserves the facilitation condition exactly.
    ...
    \item \textbf{Decoherence robustness:} The avalanche timescale $T_a \sim 10\,\mu$s is shorter than the typical Rydberg-state radiative lifetime ($\sim 100\,\mu$s for $n \sim 70$~\cite{Beterov2009}) by an order of magnitude, suggesting that the coherent regime modelled here is the relevant operating regime in practice. A full Lindblad treatment of pure dephasing has not been performed within the present work and is left to future analyses; systematic studies of comparable facilitation schemes~\cite{Festa2022} report robust amplification at moderate dephasing rates, suggesting the mechanism is not fragile to decoherence on the timescale of an avalanche event.
    \item \textbf{Detection compatibility}: State-selective field ionisation, the standard detection technique for cold Rydberg atoms \cite{Gallagher2005, Saffman2016}, achieves end-to-end efficiencies of $\eta_{\rm det} \approx 40$--70\% \cite{Ryabtsev2007, Andersen2013} and provides a single-shot detection probability of $99.3\%$ at $N=17$. For applications that demand the formal $3\sigma$ threshold, loss imaging with $\eta_{\rm det} \geq 95\%$ \cite{Tao2024, Manetsch2025} delivers $P_{\rm detect} = 99.83\%$ at $N=15$ via direct simulation. The choice between readouts is an experimental design parameter; the underlying avalanche physics is independent of either readout.
\end{enumerate}
