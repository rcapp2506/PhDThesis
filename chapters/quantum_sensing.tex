...
\begin{enumerate}
    ...
    \item \textbf{Readout} ($t = T_a$): Apply a ramped electric field pulse to selectively ionise atoms in the Rydberg state $|r\rangle$ while leaving ground-state atoms $|g\rangle$ unaffected \cite{Gallagher2005, Zhang2024Review}. The resulting ions or electrons are detected using a microchannel plate (MCP) detector with typical efficiency $\eta_{\rm det} \approx 40\%$ \cite{Ryabtsev2007, Andersen2013}. If the detected ion count exceeds $S_{\rm threshold}$, a photon event is registered.
...
\begin{enumerate}
    ...
    \item \textbf{Detection compatibility}: State-selective field ionisation, the standard detection technique for cold Rydberg atoms \cite{Gallagher2005, Saffman2016}, achieves efficiencies of $\eta_{\rm det} \approx 40$--70\% \cite{Ryabtsev2007, Andersen2013}. While lower than ground-state imaging fidelities ($>95\%$) demonstrated in current tweezer experiments \cite{Tao2024, Manetsch2025}, the amplification factors demonstrated here ($\mathcal{A} \approx 5.3$) provide sufficient signal enhancement to maintain single-photon sensitivity across the full magnetic field range. The finite detection efficiency affects only the readout stage and does not alter the underlying avalanche physics.
\end{enumerate}
