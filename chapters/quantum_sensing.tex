...
The two readout technologies relevant to the present scheme are state-selective field ionisation (SSFI) and loss imaging. Their performance differs substantially.
\begin{itemize}
    \item \emph{State-selective field ionisation (SSFI)} subjects the Rydberg atoms to a controlled electric-field ramp that ionises the population, with the resulting ions or electrons being directed to a microchannel-plate (MCP) detector. SSFI is the standard technique in cold-Rydberg experiments~\cite{Gallagher2005,Saffman2016} and is the readout assumed throughout this chapter. Its end-to-end efficiency is set by the product of MCP collection ($60$--$80\%$), ionisation completeness ($90$--$95\%$), MCP quantum efficiency ($60$--$80\%$), and incidental losses, yielding $\eta_{\rm det} \approx 40$--$70\%$ in state-of-the-art implementations~\cite{Balewski2015, Tao2024}.
    \item \emph{Loss imaging} works in the opposite direction: a short Rydberg-removing pulse first depletes the Rydberg-state population, then the surviving ground-state atoms are imaged by absorption or fluorescence, and the missing sites are counted. Because all losses now occur in a single-photon-scattering step on the cycling ground-state transition, end-to-end efficiencies $\eta_{\rm det} \geq 95\%$ are routinely achieved~\cite{Endres2016, Bluvstein2024}. The price is a more complex experimental sequence: an extra blowout pulse, an imaging beam, and accurate calibration of the depletion fidelity.
\end{itemize}
