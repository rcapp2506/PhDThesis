...
Equation~\eqref{eq:amplification_time} should be read as a fixed readout \emph{convention} rather than as the location of the signal maximum. In the exact simulations the first maximum of $S(t)$ --- the end of the ballistic phase, when the two avalanche fronts reach the chain boundaries --- occurs earlier, at $\Omega_{\mathrm{gr}} T_a^{\mathrm{peak}} \approx 0.34\,N$, i.e.\ roughly a factor of three before the convention time. Reading the same $S(t)$ at the measured peak yields $\mathcal{A}^{\mathrm{peak}} \approx 7.0$ at $N=11$, against $\mathcal{A} \approx 5.3$ at the convention time; the values quoted in this chapter use the convention readout and are in this sense conservative. The data-driven readout time, the small-$N$ behaviour of the amplification, and the effect of dephasing on both are analysed in Sec.~\ref{sec:smallN_readout}.
...
where:
\begin{itemize}
    \item $g_{a\gamma\gamma}$ is the axion-photon coupling. For QCD-band
    axions it is tied to the mass through
    $g_{a\gamma\gamma} = (\alpha/2\pi f_a)\,|E/N - 1.92|$ with
    $m_a = 5.69~\mu\mathrm{eV} \times (10^{12}~\mathrm{GeV}/f_a)$;
    the two standard benchmarks are KSVZ ($E/N = 0$) and DFSZ
    ($E/N = 8/3$), whose values at the working mass are listed in
    Table~\ref{tab:axion_benchmarks}
    \item $m_a$ is the axion mass
    \item $\rho_{\mathrm{DM}} = 0.45$ GeV/cm$^3$ is the local dark matter
    density, in the reporting convention adopted by the haloscope
    collaborations (ADMX, HAYSTAC, ORGAN)~\cite{Zhong2018, Backes2021} so
    that sensitivities are directly comparable across experiments;
    astrometric determinations span $0.3$--$0.5$ GeV/cm$^3$, and all rates
    below scale linearly with $\rho_{\mathrm{DM}}$
...
Equation~\eqref{eq:axion_power} coincides with the standard haloscope
expression (e.g.\ HAYSTAC~\cite{Zhong2018}) at critical antenna coupling,
where the extracted power fraction $\beta/(1+\beta)$ equals $1/2$; in the
present architecture the antenna-extraction step is replaced by in-cavity
atomic absorption, whose efficiency is treated separately in
Sec.~\ref{sec:backaction}.

The benchmark axion mass is chosen to match the seeding transition of the
simulated system: the dipole-allowed pair
$\ket{68S_{1/2}} \to \ket{70P_{3/2}}$ lies at $56.7$~GHz (rubidium quantum
defects), corresponding to
\begin{equation}
    m_a = 234.6~\mu\mathrm{eV}, \qquad f = m_a c^2/h = 56.7~\mathrm{GHz},
\end{equation}
squarely inside the post-inflationary QCD-axion mass window targeted by the
high-mass haloscope programme. With the cavity parameters listed above
($B = 5$~T, $V = 1$~L, $C = 0.5$, $Q = 10^5$) and the QCD-band couplings at
this mass, Eq.~\eqref{eq:axion_power} gives:

\begin{table}[htbp]
\centering
\caption{Axion-photon coupling benchmarks at the working mass
$m_a = 234.6~\mu$eV, with the corresponding on-resonance conversion power
(Eq.~\eqref{eq:axion_power}, critical coupling) and photon rate
$R_\gamma = P_{\mathrm{conv}}/hf$. The formula and its numerical
implementation are validated against the published HAYSTAC benchmark
($P \simeq 5\times10^{-24}$~W at $m_a \approx 24~\mu$eV, KSVZ, with the
HAYSTAC cavity parameters)~\cite{Zhong2018, Backes2021}.}
\label{tab:axion_benchmarks}
\begin{tabular}{lccc}
\hline\hline
Model & $g_{a\gamma\gamma}$ (GeV$^{-1}$) & $P_{\mathrm{conv}}$ (W) & $R_\gamma$ (s$^{-1}$) \\
\hline
KSVZ & $9.2\times10^{-14}$ & $9.4\times10^{-23}$ & $2.5$ \\
DFSZ & $3.6\times10^{-14}$ & $1.4\times10^{-23}$ & $0.38$ \\
\hline\hline
\end{tabular}
\end{table}

\begin{equation}
    P_{\mathrm{conv}} \approx 9.4\times10^{-23} \text{ W \quad (KSVZ benchmark)}
    \label{eq:axion_power_typical}
\end{equation}

corresponding to a photon production rate

\begin{equation}
    R_\gamma = \frac{P_{\mathrm{conv}}}{h f} \approx 2.5 \text{ photons/s
    \quad (KSVZ; } 0.38 \text{ s}^{-1} \text{ for DFSZ)}.
    \label{eq:photon_rate}
\end{equation}

Detection at these rates is exceptionally challenging: the corresponding
power lies orders of magnitude below the noise floor of conventional
radio-frequency receivers and demands single-photon-sensitive readout
combined with extremely low background rates.\footnote{All values in
Table~\ref{tab:axion_benchmarks} are computed from first principles by the
detector-parameter module in the \texttt{qh\_sensing} repository, which
includes a self-validation against the published HAYSTAC expected signal
power; an earlier version of this estimate, based on a hard-coded power
normalisation, quoted $R_\gamma \approx 24$~s$^{-1}$ at
$g_{a\gamma\gamma} = 10^{-15}$~GeV$^{-1}$ and $m_a = 20~\mu$eV, and is
superseded by the present calculation.}
...
The relevant figure of merit is the ratio of the total thermal background to the axion photon rate~\eqref{eq:photon_rate}; with the parameters above one obtains $N\,\Gamma_{\mathrm{thermal}}/R_\gamma \approx 4.4\times10^{-3}$ at the KSVZ benchmark ($2.9\times10^{-2}$ for DFSZ), i.e.\ the thermal background lies more than two orders of magnitude below the signal rate for KSVZ couplings, and a factor $\sim\!30$ below it in the more conservative DFSZ case, making it subdominant for single-shot detection. Further cooling to $T\!\sim\!300$~mK reduces $\Gamma_{\mathrm{thermal}}$ by an additional factor of $\sim\!10^{8}$.
...
Seeding target state & $|e\rangle$ & $|70P_{3/2}\rangle$ ($|r\rangle\!\to\!|e\rangle$ at 56.7 GHz) \\
...
Axion mass (benchmark) & $m_a$ & 234.6 $\mu$eV ($m_a c^2/h = 56.7$ GHz) \\
...
Panel (a) of Figure~\ref{fig:B_field_scan} shows the time evolution of $S(t)$ for four representative magnetic field values. In all cases, we observe clear ballistic growth starting from the initial single-atom excitation $S(0) = 1$. The growth rate is approximately linear up to $t \sim T_a$, consistent with Eq.~\eqref{eq:ballistic_growth}. At later times the growth stops because the available ground-state atoms are depleted: in the coherent evolution shown here the signal reaches a first maximum and then exhibits finite-size revivals (boundary reflections of a closed system); pure dephasing damps these revivals into a genuine plateau, as analysed in Sec.~\ref{sec:smallN_readout}. The slight reduction in final amplitude at higher fields reflects the suppression of $V_{rr}(B)$ according to Eq.~\eqref{eq:interaction_suppression}.
...
extending the accessible band to a few GHz for $n \gtrsim 90$. The working
point of this chapter, $m_a = 234.6~\mu$eV ($56.7$~GHz), corresponds to the
$\ket{68S_{1/2}} \to \ket{70P_{3/2}}$ pair of the simulated system. Together
...
\subsubsection{Small-$N$ Behaviour, Data-Driven Readout Time, and Dephasing}
\label{sec:smallN_readout}

The linear fit of Fig.~\ref{fig:N_scaling}(a) is a large-$N$ parametrisation, obtained from simulations at $N \geq 11$, and its domain of validity deserves to be stated explicitly. To this end we have extended the exact coherent simulation --- same Hamiltonian, parameters and central-seed initial state as in the production runs, at $B=0$ --- down to the smallest arrays, $N = 1$--$14$. The result is shown in Fig.~\ref{fig:smallN_readout}(a). Below $N \approx 8$ the amplification turns sub-linear and bends continuously towards $\mathcal{A}(1) = 1$: a single atom has no neighbour to facilitate, hence no avalanche and no gain. The positive intercept of the linear fit ($a = 2.31$) is therefore a feature of the large-$N$ parametrisation only and must not be extrapolated below its fitting domain; the extrapolated line over-predicts the gain at small $N$, crossing the detection threshold $\mathcal{A} = 3$ already at $N \approx 2$--$3$, whereas the exact curves cross it at $N \gtrsim 4$ (peak readout) and $N \gtrsim 7$ (convention readout). The production sizes $N = 11$--$15$ used throughout this chapter sit safely inside the linear regime.

Figure~\ref{fig:smallN_readout}(b) compares the two readout times on the same footing. The convention $T_a = N/\Omega_{\mathrm{gr}}$ of Eq.~\eqref{eq:amplification_time} grows three times faster than the measured position of the first maximum of $S(t)$, which follows $\Omega_{\mathrm{gr}} T_a^{\mathrm{peak}} \approx 0.34\,N$ across the full range. The measured readout time is the end of the ballistic phase --- the moment the two avalanche fronts reach the chain boundaries --- and is the natural operating point of a detector based on this scheme: it maximises the signal and, as shown next, it is also the most robust point against dephasing.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{chapters/qs_figures/amplification_smallN_readout.png}
    \caption{Small-$N$ amplification and readout time (exact coherent simulation, $B=0$). \textbf{(a)} Amplification factor $\mathcal{A}$ versus array size $N = 1$--$14$ for the two readout conventions: at the first maximum of $S(t)$ (red circles) and at the fixed convention time $T_a = N/\Omega_{\mathrm{gr}}$ (blue squares). The grey solid line is the production fit $\mathcal{A} = 2.31 + 0.286\,N$ of Fig.~\ref{fig:N_scaling}(a), drawn over its fitting domain $N \geq 11$; the grey dashed line is its extrapolation below that domain. The black dashed line marks the detection threshold $\mathcal{A} = 3$. \textbf{(b)} Readout time versus $N$: fixed convention $T_a = N/\Omega_{\mathrm{gr}}$ (grey squares) and measured first maximum of $S(t)$ (blue circles), which follows $\Omega_{\mathrm{gr}} T_a \approx 0.34\,N$.}
    \label{fig:smallN_readout}
\end{figure}

The coherent traces of Fig.~\ref{fig:B_field_scan}(a) do not, strictly speaking, saturate: after the first maximum the signal oscillates, since in a closed finite chain the excitation reflects off the boundaries and periodically refocuses (finite-size revivals). To establish which features of the coherent picture survive in the presence of decoherence we have solved the Lindblad master equation with pure dephasing, $L_j = \sqrt{\Gamma_\phi}\,\sigma_z^{(j)}$ on every site, exactly (QuTiP \texttt{mesolve}) for arrays up to $N = 7$, cross-validated against an independent split-operator integrator to two decimal places. The outcome, shown in Fig.~\ref{fig:avalanche_dephasing}, is twofold. First, moderate dephasing damps the revivals into a genuine plateau at $\mathcal{A}_{\mathrm{plateau}} \approx 0.42\,N$: the ``saturation'' commonly referred to in this context is, in the present closed-system simulations, the first maximum of a coherent oscillation, and becomes a true plateau only through dephasing. Second, the early amplification peak is remarkably robust: at $\Gamma_\phi = 0.1\,\Omega_{\mathrm{gr}}$ it is reduced by less than $10\%$ (e.g.\ $4.72 \to 4.36$ at $N = 7$), because the peak is reached at $T_a^{\mathrm{peak}} \approx 0.34\,N/\Omega_{\mathrm{gr}}$, well before dephasing has time to act. At $\Gamma_\phi \sim \Omega_{\mathrm{gr}}$ peak and plateau merge, consistent with the $\Gamma_\phi T_a \sim 1$ crossover towards the classical, directed-percolation-like regime characterised in driven Rydberg gases~\cite{Helmrich2020}, and in line with the robustness of facilitation dynamics to moderate dephasing reported experimentally~\cite{Festa2022}. The practical consequence is that the early, data-driven readout is preferable on both counts: it collects the largest signal and it is the least exposed to decoherence, whereas waiting until the convention time $N/\Omega_{\mathrm{gr}}$ costs signal in the dephased dynamics as well. Radiative and blackbody-induced decay during the avalanche are not included in this treatment and remain for future work. The scripts and raw data underlying this section are archived alongside the avalanche simulation code.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{chapters/qs_figures/avalanche_dephasing.png}
    \caption{Avalanche dynamics under pure dephasing (Lindblad, QuTiP \texttt{mesolve}). \textbf{(a)} Total signal $S(t)$ at $N = 7$ for dephasing rates $\Gamma_\phi/\Omega_{\mathrm{gr}} = 0, 0.1, 0.5, 1$; markers indicate the maximum of each trace. \textbf{(b)} Peak amplification in the coherent limit (blue circles) and at $\Gamma_\phi = 0.1\,\Omega_{\mathrm{gr}}$ (green squares), and late-time plateau amplification at $\Gamma_\phi = 0.1\,\Omega_{\mathrm{gr}}$ (orange triangles), versus array size $N = 2$--$7$. The black dashed line marks the detection threshold $\mathcal{A} = 3$.}
    \label{fig:avalanche_dephasing}
\end{figure}
...
\subsubsection{Cavity Thermal Occupation}

The thermal-background model of Eq.~\eqref{eq:thermal_rate} accounts for
spurious \emph{atomic} excitations driven by black-body radiation. A distinct
background channel, not included in the simulations, is the thermal
occupation of the \emph{cavity mode itself} at the seeding frequency: a
thermal photon resident in the mode would seed an avalanche indistinguishable
from an axion-converted one. At $f = 56.7$~GHz the Bose--Einstein occupation
is
\begin{equation}
    \bar{n}_{\rm cav}(T) = \frac{1}{e^{hf/k_BT}-1}
    \;=\; 1.0,\ 7.0\times10^{-2},\ 1.2\times10^{-4}
    \quad \text{at } T = 4,\ 1,\ 0.3~\mathrm{K},
\end{equation}
so that single-photon counting requires the cavity mode to be held at
$\bar{n}_{\rm cav} \ll 1$, i.e.\ an effective mode temperature
$\lesssim 0.6$~K (for $\bar{n}_{\rm cav} \leq 10^{-2}$)---the same
requirement faced by every single-photon haloscope readout at these
frequencies, and one reason such readouts are typically operated at
sub-Kelvin mode temperatures. Two remarks delimit the scope of this caveat.
First, the present chapter studies the physics of the avalanche
amplification mechanism in the $4$~K thermal regime, not the engineering of
a deployed detector: the avalanche dynamics, the magnetic-field robustness,
and the atomic dark-count analysis presented above are unaffected, and the
temperature scan of Table~\ref{tab:temperature_scan} already covers the
sub-Kelvin range where the cavity-mode occupation becomes negligible.
Second, the choice of the working mass mitigates this channel considerably:
at the low-frequency benchmark sometimes adopted for haloscope readouts
($\sim 5$~GHz), the same occupation at $4$~K would be
$\bar{n}_{\rm cav} \approx 17$, whereas at $56.7$~GHz it is already of order
unity, and an order-of-magnitude reduction in temperature suffices to
suppress it.

\subsubsection{Single-Shot vs. Integrated Detection}

The simulations model the single-shot detection of individual photons. In practice, axion experiments integrate over long times ($\sim 100$ s) to accumulate signal. The avalanche detector must be reset after each detection cycle, limiting the maximum detection rate to $\sim 1/(T_a + T_{\mathrm{reset}}) \sim 10$ kHz. For the axion photon rates relevant here, $R_\gamma\!\sim\!\mathcal{O}(1)$~s$^{-1}$ (cf.\ Eq.~\ref{eq:photon_rate}), this maximum detection rate is more than adequate.
...
The amplification factor at $N=11$ atoms is $\mathcal{A} \approx 5.3$, and remains essentially constant as the external magnetic field is scanned from $0$ to $5$~T, with the four sampled field values overlapping to within $1\%$ at every $N$ in the range examined. Direct simulation at $N = 11, 13, 15, 17$ establishes a linear scaling $\mathcal{A}(N) = 2.31 + 0.286\,N$ that holds uniformly across the same field range (and, per the exact small-$N$ scan of Sec.~\ref{sec:smallN_readout}, only above $N \approx 8$); the underlying mechanism is the laser auto-detuning, which absorbs the Zeeman shift of the driven transition and preserves the facilitation condition. The thermal background, exponentially suppressed in $\Delta E/k_BT$, falls below $0.01$~Hz at $T = 4$~K and below $10^{-4}$~Hz at $T = 300$~mK; these figures sit in the range demonstrated for the leading superconducting single-photon technologies operated at sub-Kelvin temperatures, and the present analysis suggests that they remain accessible at the $4$~K operating point examined here.

The avalanche timescale is short on every relevant clock --- $T_a \sim 10~\mu$s at the fixed readout convention, and a factor $\sim 3$ shorter at the measured first maximum of the signal (Sec.~\ref{sec:smallN_readout}) --- against a typical Rydberg-state radiative lifetime at $n\sim 70$ of $\sim 100~\mu$s~\cite{Beterov2009}, so the coherent regime modelled in this chapter is the relevant operating regime in practice. A Lindblad treatment of pure dephasing is presented in Sec.~\ref{sec:smallN_readout}: moderate dephasing ($\Gamma_\phi = 0.1\,\Omega_{\mathrm{gr}}$) leaves the early amplification peak within $\sim 10\%$ of its coherent value while damping the finite-size revivals into a plateau, quantitatively confirming the robustness to moderate dephasing reported for comparable facilitation schemes~\cite{Festa2022}. The avalanche additionally produces a V-shaped spatial signature in the excitation density that provides an internal discriminator against background events, as discussed in Sec.~\ref{sec:axion_benchmarking}. State-selective field ionisation, the standard detection technique for cold Rydberg atoms~\cite{Gallagher2005, Saffman2016}, has demonstrated end-to-end efficiencies $\eta_{\rm det} \approx 40$--$70\%$~\cite{Ryabtsev2007, Andersen2013}, sufficient to deliver a single-shot detection probability of $99.3\%$ at $N=17$ in our simulations; loss imaging at $\eta_{\rm det} \geq 95\%$~\cite{Tao2024, Manetsch2025} reaches $P_{\rm detect} = 99.83\%$ at $N=15$ and would satisfy the formal $3\sigma$ threshold. The choice between the two readouts is an experimental design parameter; the avalanche physics analysed in this chapter is independent of either.
...
The model developed here is a starting point, not an experimental result. Several steps would be needed before the predictions of this chapter could be tested against a working detector. Integrating a cold-Rydberg platform with a cryogenic microwave cavity is the first of these, both because it determines whether the optical access required for trapping and readout is compatible with the cavity geometry, and because cavity losses and photon-injection coupling enter the detection efficiency directly. A demonstration of the avalanche amplification in the presence of magnetic fields up to $5$--$7$~T would then test the central prediction of the present analysis. Characterisation of dark count rates under realistic experimental conditions, measurement of photon absorption efficiency, and the development of single-atom imaging techniques compatible with cryogenic and high-field environments are the natural follow-ups; none of them is a small undertaking on its own, and the predictions of this chapter would have to be revisited as data become available.
 ...
 The treatment developed in this chapter is predominantly coherent (unitary), and on that basis it establishes four results: the facilitation-driven amplification mechanism and its leading-order magnitude ($\mathcal{A}\approx5.3$ at $N=11$ at the fixed readout convention, $\mathcal{A}\approx7.0$ at the measured first maximum, scaling as $\mathcal{A}=2.31+0.286\,N$ in the operating regime $N \geq 11$ and bending to $\mathcal{A}(1)=1$ at small $N$, Sec.~\ref{sec:smallN_readout}); the field-independence of the amplification across $0$--$5$~T, which follows from the laser auto-detuning at the level of the Hamiltonian and is therefore insensitive to dissipation; the ballistic avalanche timescale --- $T_a\sim N/\Omega_{\mathrm{gr}}$ as a convention, $\Omega_{\mathrm{gr}}T_a\approx0.34\,N$ at the measured peak --- and the V-shaped spatial signature; and the single-shot detection probabilities with the associated array-size thresholds for $3\sigma$ confidence. The pure-dephasing part of the open-system problem has now been computed directly (Sec.~\ref{sec:smallN_readout}): the dephasing correction to the amplification peak is below $10\%$ at $\Gamma_\phi=0.1\,\Omega_{\mathrm{gr}}$; the merging of peak and plateau at $\Gamma_\phi\sim\Omega_{\mathrm{gr}}$ is consistent with the $\Gamma_\phi T_a\sim1$ crossover to the classical, directed-percolation regime characterised experimentally in driven Rydberg gases~\cite{Helmrich2020}; and a robustness curve $\mathcal{A}(\Gamma_\phi)$ up to $N=7$ replaces the qualitative robustness previously inferred from comparable facilitation experiments~\cite{Festa2022}. What remains for follow-up work is a self-consistent account of radiative and blackbody-induced decay during the avalanche, and the extension of the dephasing scan to the production array sizes. Such refinements would delimit the regime of validity of the present predictions further, while leaving unchanged the field-independence and the dark-count budget, neither of which is sensitive to the coherent-versus-dissipative distinction.
