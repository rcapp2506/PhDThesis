...
\begin{figure}[hbp!]
    ...
    \caption{Comprehensive performance analysis of the Rydberg avalanche detector. \textbf{(a)} Single-photon amplification factor at optimal time $T_a$ versus magnetic field. The amplification is approximately constant, $\mathcal{A} \approx 5.3$, across the full $0$--$5$~T range and exceeds the detection threshold (dashed line at 3.0) by a comfortable margin, confirming single-photon sensitivity. \textbf{(b)} Total array-level dark count rate $N\,\Gamma_{\mathrm{thermal}}$ ($N=11$) versus magnetic field at $T=4$~K. The rate is essentially flat, $\sim 1.1\!\times\!10^{-2}$~Hz, well below the $1$~Hz nominal target (dashed orange). \textbf{(c)} Optimal detection time $T_a$ versus magnetic field. The state-mixing-induced reduction of $\Omega_{\mathrm{gr}}$ extends $T_a$ from $\sim 8.8~\mu$s at $B=0$ to $\sim 12.5~\mu$s at $B=5$~T (a $\sim 40\%$ increase), still well within the timing tolerance of standard experimental electronics. \textbf{(d)} Facilitation condition residual $|\Delta_{\mathrm{gr}}(B) + V_{rr}(B)|/(2\pi)$ versus magnetic field, plotted with a $10^{-4}$~MHz floor for visualisation. After Zeeman correction the residual is identically zero at every $B$, indicating exact preservation of the facilitation condition through laser retuning. System parameters as in Fig.~\ref{fig:B_field_scan}.}
...
Panel (b) explores the array-level dark count rate $N\,\Gamma_{\mathrm{thermal}}$. At $T = 4$ K, a practical operating temperature achievable with closed-cycle pulse-tube cryocoolers, the rate is already competitive with state-of-the-art superconducting detectors and lies almost two orders of magnitude below the $1$~Hz nominal target. As shown by the temperature scan in Tab.~\ref{tab:temperature_scan}, further cooling to $T \sim 300$ mK (achievable with $^3$He refrigeration) reduces the array-level rate to $\sim 6.5\!\times\!10^{-11}$~Hz, making thermal background entirely negligible compared to the axion photon rate.
